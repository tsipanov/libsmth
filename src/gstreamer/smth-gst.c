 ...
 * \internal
 * \file smth-gst.c
 * \brief GStreamer plugin
 * \date 23rd December 2010
 * \author Stefano Sanfilippo
 *
 * Inspired from libmms gstreamer plugin by Maciej Katafiasz
 * <mathrick@users.sourceforge.net>
 */
...
enum { PROP_0, PROP_LOCATION, PROP_CONNECTION_SPEED };
...
static GstStaticPadTemplate src_factory = GST_STATIC_PAD_TEMPLATE ("src",
	GST_PAD_SRC,
	GST_PAD_ALWAYS,
	GST_STATIC_CAPS ("video/x-ms-smth")
	);

static void
gst_smth_urihandler_init (GType mms_type)
{
  static const GInterfaceInfo urihandler_info = {
	gst_smth_uri_handler_init,
	NULL,
	NULL
  };
...
GST_BOILERPLATE_FULL (GstMMS, gst_smth, GstPushSrc, GST_TYPE_PUSH_SRC,
	gst_smth_urihandler_init);

static void
gst_smth_base_init (gpointer g_class)
{
...
static void
gst_smth_class_init (GstMMSClass * klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstBaseSrcClass *gstbasesrc_class = (GstBaseSrcClass *) klass;
  GstPushSrcClass *gstpushsrc_class = (GstPushSrcClass *) klass;

  gobject_class->set_property = gst_smth_set_property;
  gobject_class->get_property = gst_smth_get_property;
  gobject_class->finalize = gst_smth_finalize;
  ...
  gstbasesrc_class->start = GST_DEBUG_FUNCPTR (gst_smth_start);
  gstbasesrc_class->stop = GST_DEBUG_FUNCPTR (gst_smth_stop);

  gstpushsrc_class->create = GST_DEBUG_FUNCPTR (gst_smth_create);

  gstbasesrc_class->is_seekable = GST_DEBUG_FUNCPTR (gst_smth_is_seekable);
  gstbasesrc_class->get_size = GST_DEBUG_FUNCPTR (gst_smth_get_size);
  gstbasesrc_class->prepare_seek_segment =
	  GST_DEBUG_FUNCPTR (gst_smth_prepare_seek_segment);
  gstbasesrc_class->do_seek = GST_DEBUG_FUNCPTR (gst_smth_do_seek);
  gstbasesrc_class->query = GST_DEBUG_FUNCPTR (gst_smth_query);
}
 ...
 */
static void
gst_smth_init (GstMMS * mmssrc, GstMMSClass * g_class)
{
...
static void
gst_smth_finalize (GObject * gobject)
{
...
static gboolean
gst_smth_query (GstBaseSrc * src, GstQuery * query)
{
...
static gboolean
gst_smth_prepare_seek_segment (GstBaseSrc * src, GstEvent * event,
	GstSegment * segment)
{
...
static gboolean
gst_smth_do_seek (GstBaseSrc * src, GstSegment * segment)
{
  mms_off_t start;
  GstMMS *mmssrc = GST_MMS (src);
...
gst_smth_create (GstPushSrc * psrc, GstBuffer ** buf)
{
...
static gboolean
gst_smth_is_seekable (GstBaseSrc * src)
{
  GstMMS *mmssrc = GST_MMS (src);
...
static gboolean
gst_smth_get_size (GstBaseSrc * src, guint64 * size)
{
  GstMMS *mmssrc = GST_MMS (src);
...
static gboolean
gst_smth_start (GstBaseSrc * bsrc)
{
...
static void
gst_smth_set_property (GObject * object, guint prop_id,
	const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
	case PROP_LOCATION:
	  gst_smth_uri_set_uri (GST_URI_HANDLER (mmssrc),
...
static void
gst_smth_get_property (GObject * object, guint prop_id,
...
static GstURIType
gst_smth_uri_get_type (void)
{
  return GST_URI_SRC;
}

static gchar **
gst_smth_uri_get_protocols (void)
{
...
static const gchar *
gst_smth_uri_get_uri (GstURIHandler * handler)
{
...
static gchar *
gst_smth_src_make_valid_uri (const gchar * uri)
{
  gchar *protocol;
...
static gboolean
gst_smth_uri_set_uri (GstURIHandler * handler, const gchar * uri)
{
  GstMMS *src = GST_MMS (handler);
  gchar *fixed_uri;

  fixed_uri = gst_smth_src_make_valid_uri (uri);
  if (!fixed_uri && uri)
	return FALSE;
...
static void
gst_smth_uri_handler_init (gpointer g_iface, gpointer iface_data)
{
  GstURIHandlerInterface *iface = (GstURIHandlerInterface *) g_iface;

  iface->get_type = gst_smth_uri_get_type;
  iface->get_protocols = gst_smth_uri_get_protocols;
  iface->get_uri = gst_smth_uri_get_uri;
  iface->set_uri = gst_smth_uri_set_uri;
}
